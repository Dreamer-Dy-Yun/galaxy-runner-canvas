...
- `session-input.test.mjs`: action/repeat/transient, blur·visibility reset version, listener 해제, 기본 기체 출격, route 초기화, Restart, Assist Continue, pause 상태 전이를 검증한다.
- `run-rules.test.mjs`: 시작 무기 순서·정규화·순환·직접 선택과 Assist 판정을 검증한다.
- `opening-route-choice.test.mjs`: 1초 기본 기체 노출, 고정 4종 선택, 단일 선택/전체 제거, rig adapter hook, 이후 weapon 생성·morph route lock을 검증한다.
...
- `browser-smoke.mjs`: build artifact에서 debug off/on, base launch → route choice → combat → special feedback → pause/info → restart-ready, 접근성·mute·blur 직후 special 재입력을 검증한다.
