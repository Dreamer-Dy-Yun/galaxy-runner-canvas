...
test("main publishes a frozen read-only status snapshot without gameplay controls", async () => {
  ...
  const globals = {
    ...
    document: {
      ...
      getElementById(id) { return id === "game" ? canvas : restartButton; },
  ...
  const placeholderGlobals = [
    "AssetLoader", "AssetPreloader", "RenderHelpers", "SpriteAtlas", "Scene",
    "ActionMap", "InputState", "EntityStore", "EntityGroups", "World",
    "CollisionQuery",
  ];
  for (const name of placeholderGlobals) context[name] = class Placeholder {};
  ...
  context.Game = class Game {
    constructor() {
      this.state = { mode: "ready", distance: 12.5, score: 42 };
      this.player = { health: 7 };
      this.world = { groups: new Map() };
    }
  };
  ...
  assert.equal(typeof context.GalaxyRunnerStatus, "function");
  const status = context.GalaxyRunnerStatus();
  assert.equal(Object.isFrozen(status), true);
  assert.equal(status.mode, "ready");
  assert.equal(status.distance, 12.5);
  assert.equal(status.score, 42);
  assert.equal(status.hp, 7);
