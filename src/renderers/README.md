...
- `player-renderer.js`: shield, special, thruster, rig ship의 고정 draw 순서.
- `player-rig-art.js`: rig assetKey 해석, 공통 rig renderer 호출, 승인된 final-form/기본 기체 fallback.
- `player-part-layout.js`, `final-ship-art.js`: armor/drone 지원 atlas와 승인된 final-form fallback.
...
- player renderer는 전체 기체에 non-uniform scale/skew를 적용하지 않으며 rig frame의 rigid pose만 소비한다.
